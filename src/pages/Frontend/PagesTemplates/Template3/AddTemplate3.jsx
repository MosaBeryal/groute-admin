...
import barcodeservice from "../../../../Images/NewUI/industrysolutions.webp";
...
const AddTemplate3 = ({ isVisible, setVisibility, refreshTemplate3Data }) => {
  ...
  const handleFileChange = (e, fileField) => {
    ...
    } else if (fileField === "section_2_image") {
      setSectionImage2(file);
    } else if (fileField === "section_3_image") {
  ...
  const handleSubmit = async () => {
    ...
    if (section_image2) {
      ...
      formData.append("sections[2][contents][0][data][fileField]", "section_2_image");
    }

    if (section_image3) {
      formData.append("section_3_image", section_image3);
      formData.append("sections[3][contents][0][data][fileField]", "section_3_image");
    }
          ...
          <div className="popup-container h-auto sm:w-[90%] w-full">
            <div className="popup-form w-full" style={{ maxHeight: "90vh", overflowY: "auto" }}>
              <div className="w-full">
                <div className={`flex justify-between w-full mb-3 ${isArabic ? 'flex-row-reverse' : 'flex-row'}`}>
                  <h2 className={`text-secondary font-sans font-semibold text-2xl ${isArabic ? "text-end" : "text-start"}`}>
                    {t("Add")} {t("Template3")}
                  </h2>
                  <button className="text-gray-500 hover:text-gray-700 focus:outline-none" onClick={handleCloseCreatePopup}>
                ...
                <div className="py-3 shadow-lg px-3 mb-3">
                  <div className="flex flex-col sm:gap-8 gap-3 sm:flex-row sm:justify-between sm:mt-0 mt-4">
                    <div className="w-full font-body sm:text-base text-sm flex flex-col gap-0">
                      <label htmlFor="fields1" className={`text-secondary ${isArabic ? "text-end" : "text-start"}`}>
                        {t("Template Name")}
                      </label>
                      <input
                        type="text"
                        id="fields1"
                        className={`border-1 rounded-sm border-[#8E9CAB] p-2 ${isArabic ? "text-end" : "text-start"}`}
                    ...
                    <div className="w-full font-body sm:text-base text-sm flex flex-col gap-0 sm:h-16 h-auto">
                      <label htmlFor="fields2" className={`text-secondary ${isArabic ? "text-end" : "text-start"}`}>
                        {t("Template")} {t("Name[Arabic]")}
                      </label>
                      <input
                        type="text"
                        id="fields2"
                        className={`border-1 w-full rounded-sm border-[#8E9CAB] p-2 ${isArabic ? "text-end" : "text-start"}`}
                        onChange={(e) => handleChange("name_ar", e.target.value)}
                        placeholder={`${t("Enter")} ${t("Template")} ${t("Name[Arabic]")}`}
                      />
                    </div>

                    <div className="w-full font-body sm:text-base text-sm flex flex-col gap-0 sm:h-16 h-auto">
                      <label htmlFor="fields3" className={`text-secondary ${isArabic ? "text-end" : "text-start"}`}>
                        {t("Slug")}
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="flex flex-col sm:gap-8 gap-3 sm:flex-row sm:justify-between sm:mt-0 mt-4">
                    <div className="w-full font-body sm:text-base text-sm flex flex-col gap-0 sm:h-auto h-auto mt-4">
                      <label htmlFor="seo_description" className={`text-secondary ${isArabic ? "text-end" : "text-start"}`}>
                        {t("SEO Description")}
                      </label>
                      <textarea
                        type="text"
                    ...
                    <div className="w-full font-body sm:text-base text-sm flex flex-col gap-0 sm:h-auto h-auto mt-4">
                      <label htmlFor="seo_description_ar" className={`text-secondary ${isArabic ? "text-end" : "text-start"}`}>
                  ...
                  <div className="font-body sm:text-base text-sm flex flex-col gap-0 mt-4 mb-3">
                    <label htmlFor="fields4" className={`text-secondary ${isArabic ? "text-end" : "text-start"}`}>
                      {t("Status")}
                    </label>
                    <select
                      id="fields4"
                ...
                <div className="relative w-full h-[450px]">
                  ...
                  <div className={`absolute sm:top-1/6 top-2.5 text-white md:px-10 px-3 ${isArabic ? 'text-right' : 'text-left'}`}>
                    <textarea
                      className="w-full h-14 bg-transparent sm:text-5xl text-3xl font-sans font-semibold mb-4 text-white"
                      onChange={(e) => handleChange("sections.0.contents.0.data.heading", e.target.value)}
                      placeholder={`${t("Enter")} ${t("Heading")}`}
                    />
                    <textarea
                      className="w-full h-14 bg-transparent sm:text-5xl text-3xl font-sans font-semibold mb-4 text-white"
                      onChange={(e) => handleChange("sections.0.contents.0.data_ar.heading", e.target.value)}
                      placeholder={`${t("Enter")} ${t("Heading[Arabic]")}`}
                    />
                    <div className="mt-3">
                      <ReactQuill
                        theme="snow"
