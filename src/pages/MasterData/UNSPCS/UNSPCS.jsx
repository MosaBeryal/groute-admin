...
const UNSPCS = () =>
{
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await newRequestnpc.get("/master-data/getAllUNSPSC");
  ...
  const handleDelete = async (row) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const isDeleted = await newRequestnpc.delete("/master-data/deleteUNSPSC/" + row?.id);
          if (isDeleted) {
            toast.success(
              `${t("UNSPSC")} ${t("Delete")} ${t("successfully")}!`,
              {
                position: "top-right",
                autoClose: 2000,
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        json.forEach((item) => {
          newRequestnpc.post(`/master-data/createUNSPSC`, {
            ...
            .then((res) => {
              ...
              Swal.fire("Add!", `UNSPCS has been created`, "success");
              refreshcitiesData();
            })
            .catch((err) => {
              // console.log(err);
              Swal.fire("Error!", `Some UNSPCS already exist`, "error");
              // Handle errors
            });
        });
      };
