...
import { crnumber__, paymentSlipColumn } from '../../../../utils/datatablesource'
import newRequest from '../../../../utils/userRequest'
import { useQuery } from 'react-query'
import Swal from 'sweetalert2';
import { toast } from 'react-toastify';
import Addcrnumber from './addcrnumber';
import Updatecrnumber from './updatecrnumber';
...
const crnumber = () =>
{
  ...
  useEffect(() =>
  {
    ...
    {
      try {
        const response = await newRequest.get("/crs",);
  ...
  const handleDelete = async (row) =>
  {
    ...
    }).then(async (result) =>
    {
      if (result.isConfirmed) {
        try {
          const isDeleted = await newRequest.delete("/crs/" + row?.id);
          if (isDeleted) {
            toast.success(`${t('Cr Number')}  ${t('has been deleted')} ${t('successfully')}!`, {
              position: "top-right",
              autoClose: 2000,
    ...
    <div>
      {/* <SideNav> */}
        <div className={`p-0 h-full bg-dashboard-color`}>
          ...
          {/* Addcrnumber component with handleShowCreatePopup prop */}
          ...
          {/* Updatecrnumber component with handleShowUpdatePopup prop */}
