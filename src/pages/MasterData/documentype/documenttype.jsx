...
const DocumentType = () =>
{
  ...
  const handleDelete = async (row) =>
  {
    Swal.fire({
      ...
      text: `${t('You will not be able to recover this')} ${t('document Type')}!`,
      icon: 'warning',
    ...
    }).then(async (result) =>
    {
      if (result.isConfirmed) {
        try {
          ...
          if (isDeleted) {
            toast.success(`${t('document Type')}  ${t('has been deleted')} ${t('successfully')}!`, {
              position: "top-right",
              autoClose: 2000,
  ...
  const handleFileUpload = (e) =>
  {
    ...
    if (file) {
      ...
      reader.onload = (e) =>
      {
        ...
        json.forEach((item) =>
        {
          newRequest.post(`/createdocumentType`, {
            file_name: item.file_name, // Adjust property names as needed
            ...
            .then((res) =>
            {
              ...
              Swal.fire(
                'Add!',
                `document Type has been created`,
                'success'
              )
              refreshcitiesData()
            })
